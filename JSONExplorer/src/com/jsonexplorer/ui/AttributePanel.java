...
public class AttributePanel extends JPanel {
	...
	public AttributePanel(JSONInheritance json_inheritance) {
		...
		if (json_inheritance != null)
			attribute_name_label.setText("Attribute: \"" + json_inheritance.getKey().toString() + "\"");
	}
	...
	public void saveChanges() {
		...
		if (json_inheritance.getParent() instanceof JSONObject) {
			jo = (JSONObject) json_inheritance.getParent();
			if (json_inheritance.getKey() instanceof String) {
				jok = (String) json_inheritance.getKey();
				if (jo.opt(jok) != null) {
					jo.put(jok, value);
					json_inheritance = new JSONInheritance(value, jok, json_inheritance.getParent(), json_inheritance.getName());
					notifier.setOnSaveChanges(new AttributePanelEventArgs(this, json_inheritance));
				}
			}
		} else if (json_inheritance.getParent() instanceof JSONArray) {
			ja = (JSONArray) json_inheritance.getParent();
			if (json_inheritance.getKey() instanceof Integer) {
				jak = (Integer) json_inheritance.getKey();
				if ((ja.length() < jak) && (jak >= 0)) {
					ja.put(jak, value);
					json_inheritance = new JSONInheritance(value, jak, json_inheritance.getParent(), json_inheritance.getName());
					notifier.setOnSaveChanges(new AttributePanelEventArgs(this, json_inheritance));
				}
			}
		}
